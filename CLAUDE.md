...
- **Ветки цепочкой**: `main → step/0-conventions → step/1-foundation → step/2-core → step/3-polish → step/4-bonus`.
  Каждая следующая ветка создаётся от предыдущей. По умолчанию Claude не мерджит в `main`, не пушит и не
  ставит теги `step/N` — автор полирует ветку, мерджит и тегирует сам. Исключение делается только по
  прямой просьбе автора; так и было сделано при сдаче 2026-09-19 (теги `step/1…4`, слияние в `main`
