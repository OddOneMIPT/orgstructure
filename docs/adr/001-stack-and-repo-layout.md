...
| ESLint              | **10.x**          | берём актуальную ветку (9.x уже помечена неподдерживаемой). Возможно только потому, что `eslint-plugin-react`, который заявляет `^9.7`, мы не используем                                                                                                       |
| eslint-plugin-react | **не используем** | нужен был только ради `react/forbid-dom-props`; тот же запрет закрывается ядром: `no-restricted-syntax` с селектором `JSXAttribute[name.name='style']` — и он ловит не только DOM-элементы, но и компоненты Побочный выигрыш — развязаны руки по версии ESLint |
