...
**Что осталось непроверенным:** успешный путь AI-поиска не выполнялся против живого API — на машине сборки
нет `ANTHROPIC_API_KEY`. Проверены ветка fallback (вручную и в браузере через Nginx), валидация тела
запроса, разбор ответа модели и предикат фильтра — на подменяемом клиенте и юнит-тестами.
