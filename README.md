...
```bash
npm install
npm run dev          # сервер :3001 и клиент :5173 одной командой
```

Нужен Node 22 (`.nvmrc`). Клиент открывается на http://localhost:5173 и ходит в API через прокси Vite —
...
- **Дерево** — раскрытие ветвей, у каждого узла имя, штат и индикатор эффективности; уровни 0 и 1
...
- «Второй уровень открыт по умолчанию» понято буквально: раскрыты и дивизионы, и отделы, поэтому третий
  уровень (команды) виден сразу.
